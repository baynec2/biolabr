...
#'
#' @return a dataframe containing the taxonomy assignments corresponding with the samples in the folder path.
#' @export
#'
#' @examples
...
Genewiz_16S = function(folder_path,
    ...
    p1 = make.consensus.seqs(folder_path,
    ...
    names(seq) = stringr::str_match(names(seq),"//(.*)-16[s,S]-")[,2]
    ...
    #Now that we have the NCBI accession number, we need to figure out what the corresponding taxa is. We can get that using the accessionTaxa.sql database.
    output = output %>%
    ...
    x = dplyr::bind_cols(taxa_call,output)
